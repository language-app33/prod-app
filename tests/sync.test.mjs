...
const shared = (extra = {}) => ({
  id: "c1", ar: "كتاب", en: "book", lat: "kitaab", tags: [], updated: 100,
  s: states(), subs: [{ id: "c1-f0", ar: "كتب", en: "books", s: states() }], ...extra,
});
const doc = (items, extra = {}) => ({
  items, tombstones: {}, log: {}, settings: {}, settingsUpdated: 0, ...extra,
});
...
test("two devices answer different exercises on one card: both answers survive", () => {
  ...
  phone.s.ar2en = { ...fresh(), reps: 3, updated: 210 };
  const laptop = shared({ updated: 300 });
  laptop.s.en2ar = { ...fresh(), reps: 7, updated: 310 };
  const out = mergeData(doc([phone]), doc([laptop])).items[0];
  assert.equal(out.s.ar2en.reps, 3, "the phone's work on one exercise");
  assert.equal(out.s.en2ar.reps, 7, "the laptop's work on another");
});
...
test("an exercise only the other device has answered is carried across", () => {
  ...
  delete untouched.s.en2ar;
  const answered = shared({ updated: 200 });
  answered.s.en2ar = { ...fresh(), reps: 6, updated: 210 };

  assert.equal(mergeData(doc([untouched]), doc([answered])).items[0].s.en2ar.reps, 6,
    "the incoming answer is kept when this device has no record of that exercise");
  assert.equal(mergeData(doc([answered]), doc([untouched])).items[0].s.en2ar.reps, 6,
...
test("two devices answer the same exercise: the later answer wins, by its own clock", () => {
  ...
  phone.s.ar2en = { ...fresh(), reps: 3, interval: 5, updated: 100 };
  const laptop = shared({ updated: 100 });
  laptop.s.ar2en = { ...fresh(), reps: 9, interval: 40, updated: 900 };
  const out = mergeData(doc([phone]), doc([laptop])).items[0];
  ...
  assert.equal(out.s.ar2en.reps, 9);
  assert.equal(out.s.ar2en.interval, 40);
});

test("progress on a form survives a text edit made on the other device", () => {
  const phone = shared({ updated: 200 });
  phone.subs[0].s.ar2en = { ...fresh(), reps: 4, updated: 250 };
  const laptop = shared({ en: "the book", updated: 300 });
  const out = mergeData(doc([phone]), doc([laptop])).items[0];
  assert.equal(out.en, "the book", "the laptop's text edit is kept");
  assert.equal(out.subs[0].s.ar2en.reps, 4, "and the phone's work on the plural is not thrown away");
});
...
test("a form added on the losing device is dropped", () => {
  ...
  const phone = shared({ updated: 200 });
  phone.subs.push({ id: "c1-f1", ar: "كتابان", en: "two books", s: states() });
  const laptop = shared({ en: "the book", updated: 300 });
  const out = mergeData(doc([phone]), doc([laptop])).items[0];
  assert.deepEqual(out.subs.map((f) => f.id), ["c1-f0"], "the added form is gone");
  ...
  assert.deepEqual(other.subs.map((f) => f.id), ["c1-f0", "c1-f1"]);
});
