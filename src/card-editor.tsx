...
import { fillsOf, fillText, hasSlots, isLent, slotsIn, slotsOf, slotTrouble, valuesFor, valuesForTurn, WORD_SLOT } from "./variables.ts";
 ...
 * read like theirs. Since 0.159 there is one question, asked of each part
...
/* ---- what is drilled, and where ----
   ...
   Two questions, since 0.159, because they were always two: whether a
...
export function useWordDraft({ card, lang, allCards, draft, shape }: {
  ...
  const [fills, setFillsChosen] = useState(((card && card.fills) || "").toLowerCase());
  /*
   * Naming a blank on a new card takes it out of the questions.
   *
   * "Raphael" is written to fill *my name is {{name}}*, and *what does
   * Raphael mean* is not a question — so a card that says it fills a
   * blank, and has never said anything else about what is drilled,
   * starts as a word that is lent and not asked. That has been the
   * default since values existed; what has changed is that it now
   * happens in the ticks, where the teacher can see it and say
   * otherwise, rather than in a hidden third state of a toggle nobody
   * was shown.
   *
   * Only on a card nobody has saved, and only the first time a blank is
   * named: a card already written has answered this, and answering it
   * again over the teacher's head is the thing this replaces.
   */
  const setFills = (next: string) => {
    setFillsChosen(next);
    if (card || !next.trim() || fills.trim()) return;
    setForms((x) => x.map(keptNotAsked));
    setCells((x) => x.map(keptNotAsked));
  };
   ...
   * card the answer is settled before anybody reaches it. A card with a
   * blank of its own is a frame, and a frame is never a filler — a
   * sentence dropped into somebody else's hole is a sentence with a gap
   * where the point was. And a card that stands in no hole — one that
   * names no blank, says it is no kind of word, and whose own word is
   * too long to be the one every frame borrows — is lent nowhere
   * whatever is ticked.
   *
   * Asked of fillsOf, which is the one answer to what a card stands in,
   * so this cannot come to disagree with what actually happens at
   * question time.
   */
  const canLend =
    !scene &&
  ...
  const asked = useMemo(() => {
    ...
    const have = valuesFor(main, allCards || [], lang && lang.id, (c) => kindOf(c, lang), (c, f) => lendsForm(lang, c)(f));
    const out: string[] = [];
  ...
  const setAskPart = (id: string, on: boolean) => setPart(id, "ask", on);
  const setLendPart = (id: string, on: boolean) => setPart(id, "lend", on);
...
/* What to call a verb whose own word is a cell of its table. */
function NameBlock({ word }: { word: WordDraft }) {
  const { shownSpec, name, setName } = word;
  if (!shownSpec || !citationOf(shownSpec)) return null;
  return (
    <>
    {/* ---- what to call it ----
...
function FormBlock({ word, lang, index: i, form: f, title, role, children }: {
  ...
  const { canSave, drillsTranslit, parts, setForm, duplicateForm, removeForm, setRecording } = word;
  ...
  <div className={`at-formblock${i === 0 ? " main" : ""}`}>
    ...
    {/* ---- the word itself ----
        ...
        0.159 the only thing saying where one ended was that line,
...
function PronounTable({ word, lang, index: i, form: f }: {
  ...
  /* ---- the pronouns this form takes on its end ----
      ...
      A subsection of that form's block since 0.159, with its name
...
function BlanksBlock({ word, lang }: { word: WordDraft; lang: Lang }) {
  const { holes, starved, asked, fills, setFills, main, standsIn, blanksAround, putBlank, trouble } = word;
  return (
    <>
    {/* ---- blanks ----
      ...
      <div className="at-formblock at-mt5">
        ...
        {(!holes.length || !!fills) && (
          <>
            ...
            {fills ? (
              /* Whether it is *also* a question of its own used to be a
                 tick here — one answer for the whole card, in a different
                 place from the per-form ticks and asking a question that
                 read like theirs. It is one of the two ticks under each
                 form now, so there is one question about what is drilled
                 and it is asked where the thing being drilled is. */
              <Help>
                Every card with a <code>{`{{${fills}}}`}</code> blank in it can borrow
                this word. Whether it is <i>also</i> asked as a question of its
                own is under the form itself, beside the word.
              </Help>
            ) : (
              <Help>
 ...
 *
 * The name only where it was asked for: a card that is not a verb of this
 * shape is named by its own word, and a name left behind from a card that
 * briefly was one would go on labelling it.
 */
export function writtenCard({ word, talk, shape, chosen }: {
  ...
  const scene = shape === "scene";
  const { shownSpec, ownForms, tableCells, forms, note, standsIn, name, uses, fills, category } = word;
...
function VerbEditor({ word, lang, allCards, selfId }: {
  ...
  return (
    <>
      {word.standsIn && <NameBlock word={word} />}
