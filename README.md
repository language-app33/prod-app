...
The app is a static build plus two Netlify functions. `netlify.toml` has the
build command and the `/api/*` redirects. Two blob stores are used, one for
the sync documents and one for course data; they are created on first write.
