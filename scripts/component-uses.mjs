...
/* The gallery is left out on purpose: it renders one of everything, so
   ...
   Named without an extension and resolved to whichever exists, because
...
function sourceOf(stem) {
  for (const ext of [".tsx", ".jsx", ".ts", ".js"]) {
    if (existsSync(path.join(ROOT, "src", stem + ext))) return "src/" + stem + ext;
  }
...
export function libraryComponents() {
  const src = readFileSync(path.join(ROOT, LIBRARY), "utf8");
...
function localNames(source, components) {
  ...
  const block = /import\s*\{([\s\S]*?)\}\s*from\s*["']\.\/shared\.[jt]sx["']/.exec(source);
  if (block) {
    ...
    lines.forEach((line, i) => {
      for (const [local, real] of names) {
        ...
        for (let n = line.matchAll(re), m = n.next(); !m.done; m = n.next()) {
          ...
          if (file === path.basename(LIBRARY) && where === real) continue;
